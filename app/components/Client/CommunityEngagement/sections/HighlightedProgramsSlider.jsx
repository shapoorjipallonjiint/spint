                ...
                {data.items.map((item, index) => (
                  <SwiperSlide key={index}>
                    <div className="relative overflow-hidden shadow-2xl" ref={imageContainerRef}>
                      <MotionImage height={576} width={1920} style={{y:imageY}} src={item.image} alt={item.imageAlt} className="w-full h-[300px] xl:h-[576px]  object-cover" />
                    </div>
                  </SwiperSlide>
                ))}
              </Swiper>
            ...
            <div className="order-1 lg:order-2   lg:w-[45%] 2xl:w-[37.5%]">
              ...
              <motion.div variants={moveUp(0.6)} initial="hidden" whileInView={"show"} viewport={{amount:0.2,once:false}} className="flex items-center gap-4   border-b border-white/20 pb-4 2xl:pb-[30px] lg:pb-6 mb-4 xl:mb-50px 3xl:mb-17">
                <button onClick={() => imageSwiper?.slidePrev()}
                  ...
                  <Image src={assets.arrowLeft2} alt="" width={14} height={14} className='w-auto h-auto group-hover:opacity-90  duration-300  transition-all delay-200'/>
                </button>
                <button onClick={() => imageSwiper?.slideNext()}
                  className="group hover:translate-x-1 transition-all duration-300 cursor-pointer w-10 h-10 xl:w-[50px] xl:h-[50px] rounded-full border border-white/20 flex items-center justify-center  "
                  aria-label="Next slide"
                >
                  <Image src={assets.arrowRight2} alt="" width={14} height={14} className='w-auto h-auto group-hover:opacity-90  duration-300  transition-all delay-200' />
                </button>

...
const Banner = ({banner,bannerAlt,pageTitle,data}) => {
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-[280px] lg:h-[350px] xl:h-[440px] 3xl:h-[560px] bg-secondary/20 overflow-hidden">
      <Image width={1500} height={1000} ref={imgRef} src={banner} alt={bannerAlt} className="absolute top-0 left-0 w-full h-full object-cover object-top z-0" />
      ...
      <div className=" relative z-2 h-full " ref={targetRef}>
        <div className="flex flex-col justify-end h-full  ">
          <div className="flex flex-col md:flex-row justify-between ps-3 gap-5 lg:gap-0">
            <h1 ref={titleRef} className="text-white text-70 xl:text-32 2xl:text-70 font-light leading-[1.08]">{pageTitle}</h1>
                ...
                {data.items.map((item, i) => (
                  <div key={i}>
                    <h3 className="text-20 lg:text-29 xl:text-32 2xl:text-40 font-light leading-[1.5] text-white">
                      {/* {item.value} */}
                      <InsideCounter value={item.number} delay={2000} /><span>+</span>
                    </h3>
                    <p className="text-14 md:text-16 lg:text-19 font-extralight leading-[1.2] text-white/70">{item.value}</p>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      <motion.div variants={moveUp(2)} initial="hidden" whileInView="show" viewport={{ amount: 0.2, once: true }} className="absolute right-0 -bottom-8 lg:bottom-0 z-10">
        <Image width={1500} height={1000} src="../assets/images/svg/sv-02.svg" className="w-[200px]  h-[230px] sm:w-[240px] sm:h-[330px] lg:w-[340px]  lg:h-[430px] 2xl:w-[449px] 2xl:h-[630px] object-cover object-center" alt="" />
      </motion.div>
    </section>
  );
};

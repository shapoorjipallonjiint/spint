            ...
            <SwiperSlide key={index} className="cursor-grab">
              <div className="relative overflow-hidden border-l border-white/30">
                <motion.div variants={moveUp(0.2*index)} initial="hidden" animate="show" viewport={{ amount: 0.6, once: true }} className="w-15 h-15 xl:w-20 xl:h-20 bg-secondary rounded-full flex items-center justify-center mb-5">
                  <Image width={20} height={20} src={item.image} alt={item.imageAlt} className="w-auto h-5 md:h-6 2xl:h-[36px] object-contain" />
                </motion.div>
                <div className="">
                  <h3 className="text-32 leading-[1.3125] font-light mb-2 xl:mb-3">{item.title}</h3>
                  <p className="text-19 leading-[1.526315789473684] font-light text-paragraph">{item.description}</p>
                </div>
              </div>
            </SwiperSlide>
          ))}

              ...
              {points.map((point, index) => {
                ...
                return (
                  <motion.div variants={moveUp(0.2*index)} initial="hidden" whileInView="show" viewport={{amount: 0.2, once: true}}
                    ...
                    className={`relative transition-all duration-300 ease-in-out  text-24 lg:text-29 leading-[1.344827586206897] cursor-pointer border-b border-black/20 last:border-b-0 py-5 xl:py-[31px]`}
                  >
                    <div className={`transition-all duration-300 ease-in-out  ${
                      isActive
                        ? "text-black translate-x-[30px]"
                        : "text-paragraph font-light"
                    }`}>
                      <span
                      className={`absolute left-0   border-l-[3px]  top-1/2 -translate-y-1/2 h-full w-[3px] transition-all duration-300 transition-width ${
                        isActive ? "translate-x-[-30px]  border-l-[3px] border-l-secondary" : "translate-x-[5px] border-l-transparent"
                      }`}
                    ></span>

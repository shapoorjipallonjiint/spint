...
const Promoters = ({ data }) => {
    ...
    const filteredTeam =
        selected.id === "all"
            ? t.dataPromoters?.items ?? []
            : (t.dataPromoters?.items ?? []).filter((member) => member.department === selected.name);

    const filteredManagement =
        selected.id === "all"
            ? t.dataManagement?.items ?? []
            : (t.dataManagement?.items ?? []).filter((member) => member.department === selected.name);

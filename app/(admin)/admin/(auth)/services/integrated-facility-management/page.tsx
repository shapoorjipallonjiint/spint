...
interface IntegratedFacilityManagementProps {
...
const IntegratedFacilityManagementPage = () => {
    ...
    const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<IntegratedFacilityManagementProps>();
    ...
    const handleAddIntegratedFacilityManagement = async (data: IntegratedFacilityManagementProps) => {
        try {
            const response = await fetch(`/api/admin/services/integrated-facility-management`, {
                method: "PATCH",
    ...
    const fetchIntegratedFacilityManagementData = async () => {
        try {
            const response = await fetch(`/api/admin/services/integrated-facility-management`);
            if (response.ok) {
                const data = await response.json();
    ...
    useEffect(() => {
        fetchIntegratedFacilityManagementData();
    }, []);


    return (
        <form className='grid grid-cols-2 gap-10' onSubmit={handleSubmit(handleAddIntegratedFacilityManagement)}>
            {/*English Version */}
                ...
                <AdminItemContainer>
                    ...
                    <div className='p-5 rounded-md flex flex-col gap-5'>
                        ...
                        <div>
                                ...
                                <div className='flex justify-end mt-2'>
                                    <Button type='button' addItem onClick={() => fourthSectionAppend({ title: "", title_ar: "", description: "", description_ar: "" })}>Add Item</Button>
                                </div>
                            </div>
...
export default IntegratedFacilityManagementPage